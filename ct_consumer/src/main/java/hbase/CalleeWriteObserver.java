...
public class CalleeWriteObserver extends BaseRegionObserver {
        ...
        public void postPut(ObserverContext<RegionCoprocessorEnvironment> e, Put put, WALEdit edit, Durability durability) throws IOException {
                ...
                calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("call1"), Bytes.toBytes(callee));
                calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("call2"), Bytes.toBytes(caller));
                calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("build_time"), Bytes.toBytes(buildTime));
                calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("flag"), Bytes.toBytes(flag));
                calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("duration"), Bytes.toBytes(duration));

                try {
                        calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("build_time_ts"), Bytes.toBytes(new SimpleDateFormat("yyyyMMddHHmmss").parse(buildTime).getTime()));
                } catch (ParseException e1) {
                        calleePut.add(Bytes.toBytes("f2"), Bytes.toBytes("build_time_ts"), Bytes.toBytes(""));
                        e1.printStackTrace();
                }
